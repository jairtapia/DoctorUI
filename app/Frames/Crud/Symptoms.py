...
import pandas as pd
from Frames.Crud.Modal.Symtom import SymptomModal
...
class SymptomsCrud(customtkinter.CTkFrame):
    def __init__(self, master,role, **kwargs):
        ...
        self.label = customtkinter.CTkLabel(master=self, text="Gestor de sitnomas", text_color="black", font=("Arial", 20))
        self.label.grid(row=0, column=0, padx=10, pady=10, sticky="w")
        self.data = self.controller.get_symptoms()
    ...
    def search(self):
        ...
        if Searchvalue.isdigit():
            ...
            valor = self.controller.search_by_id(Searchvalue)
    ...
    def openModal(self, id=None):
        if self.modal is None:
            self.modal = SymptomModal(master=self,id = id)
    ...
    def editSymptom(self,id):
        try:
            symptom = SymptomDto(
                name=self.modal.entry_nombre.get(),
                descripcion=self.modal.entry_descripcion.get()
            )
            self.controller.edit_symptom(symptom,id)
        except Exception as e:
            print(f"Error al editar el síntoma: {e}")
        finally:
            self.destroyModal()
            self.updateData()
    
    def saveSymptom(self):
        try:
            symptom = SymptomDto(
                name=self.modal.entry_nombre.get(),
                descripcion=self.modal.entry_descripcion.get()
            )
            self.controller.create_symptom(symptom)
        except Exception as e:
            print(f"Error al crear el síntoma: {e}")
        finally:
            self.destroyModal()
            self.updateData()
    ...
    def delete(self,id):
        self.controller.delete_symptom(id)
        self.updateData()
